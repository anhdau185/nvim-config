-- first override the plugin's default setup_view method
-- so that we can do some customizations on new window setup
local config = require "symbols-outline.config"
local View = require "symbols-outline.view"

View.setup_view = nil

-- overrides this method: https://github.com/simrat39/symbols-outline.nvim/blob/master/lua/symbols-outline/view.lua#L10
function View:setup_view()
  ...
  -- window stuff
  vim.api.nvim_win_set_option(self.winnr, "number", config.options.show_numbers)
  vim.api.nvim_win_set_option(self.winnr, "relativenumber", config.options.show_relative_numbers)
